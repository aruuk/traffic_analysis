 ...
 * Handles fetching from FastAPI backend
 */

const API_BASE = "http://127.0.0.1:8000/api";

// Mock data imports for fallback (kept for safety if API fails completely, usually we want to see errors now)
...
export async function fetchTrafficPrediction(minutes, bbox) {
    ...
    try {
        ...
        const response = await fetch(
            `${API_BASE}/traffic/predict?minutes=${minutes}&bbox=${bbox}`,
            {
                method: 'GET',
                headers: {
...
/**
 * Fetch optimal route from API
 * @param {Object} start - Start coordinates {lat, lng}
 * @param {Object} end - End coordinates {lat, lng}
 * @param {string} type - Transport type (car, bus, walking)
 * @returns {Promise<Object>} - GeoJSON Feature (LineString)
 */
export async function fetchOptimalRoute(start, end, type = 'car') {
    try {
